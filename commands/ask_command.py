...
用法：
    ...
    /ask -db <问题>                      → 用豆包网页提问并截图发送
    /ask -d <问题>                       → 开启深度思考后提问
    /ask -s <问题>                       → 开启联网搜索后提问
    /ask -t 展开 <问题>                  → 截图时展开思考过程块（展开/折叠/收起/reveal）
    /ask -b 收起 <问题>                  → 截图时收起左侧边栏（展开/收起/隐藏/show/hide）
    /ask -n <问题>                       → 先开新对话再提问
    /ask -g -m Pro <问题>                → 指定 Gemini 模型关键词（-m 对 Gemini/豆包生效）
    /ask -d -s -t 折叠 -b 收起 <问题>    → 组合使用
"""
...
class AiSnapshotCommand(BaseCommand):
    ...
    async def handle_ask(self, question: str) -> tuple[bool, str]:
        ...
        默认用 DeepSeek（深度思考 + 联网搜索开启）；``-g`` 切换 Gemini
        （默认模型 Flash，扩展思考由 -d 控制）；``-db`` 切换豆包（默认
        快速档，-m 专家 切深度思考档）。连续对话由同 stream 复用同一浏览器
        页面保证。

        Args:
            question: 问题文本（可带 -g/-db/-m/-d/-s/-t/-b/-r 开关前缀）。
        ...
        if not body:
            return False, "用法：/ask [-g] [-db] [-m 模型] [-d] [-s] [-t 展开/折叠] [-b 收起/展开] [-r last/full] [-c 标题/-c-] [-n] <问题>"
        ...
        if site == "doubao":
            return await self._handle_doubao(body, mode, return_scope, new_chat, conversation, config)
        return await self._handle_deepseek(body, deepthink, search, think, sidebar, return_scope, new_chat, conversation, config)
    ...
    async def _handle_gemini(
        ...
        Returns:
            tuple[bool, str]: (是否成功, 结果信息)。
        """
        # 默认模型 Flash 家族（按关键词匹配网页当前版本）；-d 控制扩展思考
        if not mode:
            mode = "Flash"
