...
- `services/base/` 放浏览器会话、共享对话定位、结果类型与页面截图能力；共享层只依赖站点动作的最小协议。
- `services/deepseek/`、`services/gemini/`、`services/doubao/` 分别拥有各自的页面动作和提问、截图、媒体服务。站点特有的页面选择器与媒体生命周期留在站点内部。
- `services/service.py` 保留已有调用方使用的服务导入路径，集中处理跨站点识图；`services/delivery.py` 负责把截图分片按顺序发到聊天。
- 新站点需要在 `config.py` 添加站点开关、在 `plugin.py` 的 `SITE_TOOLS` 装配工具，并增加对应站点服务及动作实现。`services/sites.py` 从配置字段派生有效站点名，供装配、识图和 `/ask` 判断使用。
